...
import { getAllStocksFromTV, getDataFromUser, getIndexes, getUserId } from "@/services/console"
...
const Page = async () => {
  ...
  let csv = 'Ativo,Preço-alvo,Rentabilidade,Volatilidade,Ìndice de Sharpe,Tipo\n'
  const {stocks} = await getAllStocksFromTV()
                    ...
                    return (
                      <li className="flex flex-col px-4 py-2 gap-y-2 border-2 border-black" key={key}>
                        ...
                        <p className="flex justify-between">
                          <span>Preço-alvo(Tranding View): </span>
                          <span>{stocks?.find(({ticker, target_price}) => ticker === name)?.target_price || "-" }</span>
                        </p>
                          <p className="flex justify-between">
